...
<script>
	...
	if('${param.selectDate}'!='') date = '${param.selectDate}';
						  ...
						  <c:set var="sw" value="0"/>
							<c:forEach var="i" begin="10" end="20">
							  <c:if test="${fn:length(bookingVos)!=0}">
								  <c:forEach var="j" begin="0" end="${diffSelectCount-1}">
								  	<c:if test="${sw == 0}">
									    <c:if test="${fn:substring(bookingVos[j].bookingStartDate,11,13)==i || fn:substring(bookingVos[j].bookingEndDate,11,13)==i}">
