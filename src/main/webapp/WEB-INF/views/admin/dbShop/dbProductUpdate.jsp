  ...
  <div id="product">
    ...
    <form name="myform" method="post" enctype="multipart/form-data">
      <div class="form-group">
        ...
        <select id="categoryMainCode" name="categoryMainCode" class="form-control" onchange="categoryMainChange()">
          ...
          <c:forEach var="mainVo" items="${mainVos}">
          	<option value="${mainVo.categoryMainCode}" <c:if test="${mainVo.categoryMainCode == productVo.categoryMainCode}">selected</c:if>>${mainVo.categoryMainName}</option>
          </c:forEach>
        </select>
      </div>
      <div class="form-group">         
        ...
        <select id="categorySubCode" name="categorySubCode" class="form-control">
          ...
          <option value='${productVo.categorySubCode}' selected>${productVo.categorySubName}</option>
      ...
      <div class="form-group">         
        ...
        <select id="useTime" name="useTime" class="form-control">
        	<option value="">사용시간을 선택하세요</option>
		  	  ...
		  	  <option value='${productVo.useTime}' selected>${productVo.useTime}</option>
        </select>
      </div>
      ...
      <div class="form-group">
        ...
        <c:set var="fSNames" value="${fn:split(productVo.FSName,'/')}"/>
        <div>
        	<c:forEach var="fSName" items="${fSNames}" varStatus="st">
	        	<img src="${ctp}/data/dbShop/product/${fSName}" width="100px"/>
	        </c:forEach>
        </div>
      </div>
      <div class="form-group">
      	<label for="mainPrice">상품기본가격</label>
      	<input type="text" name="mainPrice" id="mainPrice" value="${productVo.mainPrice}" class="form-control" required />
      </div>
      <div class="form-group">
      	<label for="detail">상품기본설명</label>
      	<input type="text" name="detail" id="detail" value="${productVo.detail}" class="form-control" required />
      </div>
      <!-- <div class="form-group">
